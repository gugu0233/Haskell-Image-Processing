...
toTransformation (Scaling t) = S.scaling t
toTransformation (Translation x y) = S.translation x y
toTransformation (Combine t) = S.combineTransformations (map toTransformation t)
...
toRegion (FromPoints p) = S.fromPoints p
toRegion (Rectangle w h) = S.rectangle w h
toRegion (Circle rad) = S.circle rad
toRegion (Complement region) = S.complement (toRegion region)
toRegion (Union r1 r2) = \p -> toRegion r1 p || toRegion r2 p
toRegion (Intersection r1 r2) = \p -> toRegion r1 p && toRegion r2 p
...
decomposeTransformation (Scaling t) = [Scaling t]
decomposeTransformation (Translation x y) = [Translation x y]
decomposeTransformation (Combine t) = concatMap decomposeTransformation t
...
fuseTransformations (Translation x1 y1 : Translation x2 y2 : ts) =
    fuseTransformations (Translation (x1 + x2) (y1 + y2) : ts)
fuseTransformations (Scaling x1 : Scaling x2 : xs) =
    fuseTransformations (Scaling (x1 * x2) : xs)
...
-- Funcție pentru optimizarea unei regiuni
