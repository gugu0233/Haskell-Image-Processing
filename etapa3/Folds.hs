...
instance Show TransformationAST where
    ...
    show (T (Scaling s)) = "*<" ++ show s ++ ">"
    show (T (Combine t)) = "[" ++ intercalate "," (map show t) ++ "]"
...
showRegion indentLevel (R shape) = case shape of
    FromPoints points ->
        indent indentLevel ++ "FromPoints " ++ show points ++ "\n"
    Rectangle width height ->
        indent indentLevel ++ "Rectangle " ++ show width ++ " " ++ show height
    Circle radius ->
        indent indentLevel ++ "Circle " ++ show radius ++ "\n"
    Complement region ->
        indent indentLevel ++ "~\n" ++ showRegion (indentLevel + 1) region
    Union region1 region2 ->
        indent indentLevel ++ "+\n" ++ showRegion (indentLevel + 1) region1 ++ showRegion (indentLevel + 1) region2
    Intersection region1 region2 ->
        indent indentLevel ++ "*\n" ++ showRegion (indentLevel + 1) region1 ++ showRegion (indentLevel + 1) region2
    Transform transformation region ->
        indent indentLevel ++ "" ++ show transformation ++ "\n" ++ showRegion (indentLevel + 1) region
...
foldTransformationAST f (T transformation) = case transformation of
    ...
    Scaling factor -> f (Scaling factor)
    Combine t -> f (Combine (map (foldTransformationAST f) t))
...
toTransformation = foldTransformationAST combiner
  where
    ...
    combiner (Scaling f) = S.scaling f
    combiner (Combine t) = S.combineTransformations t
