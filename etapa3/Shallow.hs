...
{-
    ...
    *****
    ...
    ..*..
-}
-- Modified plot function without using splitIntoChunks
plot :: Int -> Int -> Region -> String
plot width height region =
    intercalate "\n" [concat [if region (fromIntegral x, fromIntegral y) then "*" else "." | x <- [-width..width]] | y <-  [height, (height - 1)..(-height)]]
...
promoteBinary f pointed1 pointed2 = \x -> f (pointed1 x) (pointed2 x)
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea lenesa ajuta la evitarea calculelor inutile si genereaza reginea doar cand este necesar
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO ***
    ...
    Răspuns: Cand se verifica apartenenta unui punct care nu apartine regiunii, reuniunea infinita intoarce false, 
    pentru ca niciunul dintre cercurile din reuniune nu va contine punctul respectiv, iar transformarea prin 
    translatie nu va influenta acest aspect pentru puncte din afara regiunii initiale
...
bfs start expand = aux [(start, 0)] S.empty
  where
    aux [] _ = []
    aux ((state, dist):rest) vizitati
      | state `S.member` vizitati = aux rest vizitati
      | otherwise =
          let vecini = expand state
              newvizitati = S.insert state vizitati
              new = rest ++ [(neighbor, dist + 1) | neighbor <- vecini]
          in (state, dist) : aux new newvizitati
